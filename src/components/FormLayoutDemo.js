                ...
                <div className="card">
                    ...
                    <div className="p-fluid p-formgrid p-grid">
                        ...
                        <div className="p-field p-col-12 p-md-6">
                            ...
                            {!isEdit ? "" :  (<div><label htmlFor="tagText">Tag text/Names</label> <InputTextarea rows="5.5"  id="tagText" type="text" disabled={!isEdit} value={data && data.tagText} onChange={(e) => { console.log(e.target.value); setData({ ...data, tagText: e.target.value }) }} /></div>)}
                        </div>
                        <div className="p-field p-col-12">

                            {!isEdit ? <label>{data && data.description}</label> : (<div><label htmlFor="longDesc">Long Description</label> <InputTextarea id="longDesc" rows="8" disabled={!isEdit} value={data && data.description} onChange={(e) => { console.log(e.target.value); setData({ ...data, description: e.target.value }) }} /></div>)}
                        </div>
